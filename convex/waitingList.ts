...
export const getQueuePosition = query({
  ...
  handler: async (ctx, { eventId, userId }) => {
    ...
    //   Get people that are ahead of the user
    const peopleAhead = await ctx.db
      .query('waitingList')
      .withIndex('by_event_status', (q) => q.eq('eventId', eventId))
      .filter((q) =>
        q.and(
          // Get all entries before the user
          q.lt(q.field('_creationTime'), entry._creationTime),
          // Only gets entries that are either waiting or offered.
          q.or(
...
export const processQueue = mutation({
  ...
  handler: async (ctx, { eventId }) => {
    ...
    const waitingUsers = await ctx.db
      ...
      .order('asc')
      .take(availableSpots) // If three spots are available, we take all 3 spots instead of doing it one by use using the "take(availableSpots)"
